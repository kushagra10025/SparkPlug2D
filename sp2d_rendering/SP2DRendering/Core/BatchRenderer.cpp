...
void SP2D::Rendering::BatchRenderer::Initialize()
{
	// Generate VAO & VBO
	...
	auto indicesArray = std::make_unique<GLuint[]>(MAX_INDICES);

	for (size_t i = 0; i < MAX_INDICES; i += NUM_SPRITE_INDICES)
	{
		for (size_t j = 0; j < NUM_SPRITE_INDICES; j++)
		{
			indicesArray[i + j] = indices[j] + offset;
		}

		offset += NUM_SPRITE_VERTICES;
	}

	// Generate IBO Requires setup once only - VBO changes in Batching
	glGenBuffers(1, &m_IBO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * MAX_INDICES, indicesArray.get(), GL_DYNAMIC_DRAW);


	glBindVertexArray(0);
}

void SP2D::Rendering::BatchRenderer::GenerateBatches()
{
...
SP2D::Rendering::BatchRenderer::BatchRenderer()
...
void SP2D::Rendering::BatchRenderer::Begin()
{
	m_Sprites.clear();
	m_Batches.clear();
}

void SP2D::Rendering::BatchRenderer::End()
{
	if (m_Sprites.empty())
	{
		SP2D_CORE_INFO("No Sprites for the Batch Renderer");
		return;
	}
...
void SP2D::Rendering::BatchRenderer::Render()
{
	if (m_Batches.empty())
	{
		SP2D_CORE_INFO("No Batches for the Batch Renderer");
		return;
	}
...
void SP2D::Rendering::BatchRenderer::AddSprite(const glm::vec4& spriteRect, const glm::vec4 uvRect, GLuint textureID, int layer, glm::mat4 model, const Color& color)
{
